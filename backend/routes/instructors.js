...
router.post('/instructors', async (req, res) => {
  const { name, status, rank, qualification, years_service, salary_grade, major_ids, can_handle_ids } = req.body;
  ...
  try {
    const info = await tx.execute({
      sql: `INSERT INTO instructors (name, status, rank, qualification, years_service, salary_grade)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [name.trim(), status||'Permanent', rank||null, qualification||null, years_service||0, salary_grade||null]
    });
...
router.put('/instructors/:id', async (req, res) => {
  ...
  try {
    ...
    try {
      await tx.execute({
        sql: `UPDATE instructors SET name=?,status=?,rank=?,qualification=?,years_service=?,salary_grade=? WHERE id=?`,
        args: [name?.trim()||existing.name, status||existing.status, rank??existing.rank,
               ...
               salary_grade??existing.salary_grade, id]
      });
